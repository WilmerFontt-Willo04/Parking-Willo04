$(() => {
  ...
  $("#btnTodoParqueaderos").click(() => {
    ...
    objParqueadero.consultarTodosParqueaderosUnaVez(mostrarParqueadero);
  });
  ...
  $("#btnParqueaderoLibre").click(async () => {
    ...
    $("#parqueaderos").empty();
    const list = await parqueaderoObj.consultarParqueaderoLibres(
      mostrarParqueadero
    );
    list.forEach((parqueadero) => {
  ...
  async function obtenerTodosParqueaderos() {
    ...
    const parqueaderoObj = new Parqueadero();
    $("#parqueaderos").empty();
    await parqueaderoObj.consultarTodosParqueaderos(mostrarParqueadero);
  }
  ...
  $("#btnParqueaderosLibres").click(async () => {
    ...
    if (user) {
      ...
      Materialize.toast(
        `Debes estar autenticado para ver los parqueaderos libres`,
        4000
      );
    }
  });
  ...
  $("#btnRegistroEntrada").click(async () => {
    ...
    const imagenLink =
      sessionStorage.getItem("imgNewEntrada") == "null"
        ? null
        : sessionStorage.getItem("imgNewEntrada");

    try {
      await parqueaderoObj.crearEntrada(
        user.uid,
        idParqueadero,
        nombreCliente,
        placaCarro,
  ...
  $("#btnRegistroSalida").click(async () => {
    ...
    try {
      await parqueaderoObj.crearSalida(
        idEntrada,
        fechaSalida,
        costo,
        idParqueadero
